...
app.post('/login', (req, res) => {
  ...
  const password = req.body.password ;
    database.query('SELECT * FROM users WHERE username = ?', [username], async (err, results) => {
        ...
        if (!isMatch) {
            return res.status(401).json({ message: 'Invalid useraname or password' });
        }
...
app.get('/balance', authenticateJWT, (req, res) => {
    database.query('SELECT balance FROM users WHERE id = ?', [req.user.id], (err, results) => {
        if (err || results.length === 0) {
...
app.post('/withdraw', authenticateJWT, (req, res) => {
  ...
  database.query('UPDATE users SET balance = balance - ? WHERE id = ?', [amount, req.user.id], (err, results) => {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
        database.query('INSERT INTO transactions (user_id, type, amount) VALUES (?, "withdrawal", ?)', [req.user.id, amount]);
...
app.post('/transfer', authenticateJWT, (req, res) => {
    ...
    database.query('SELECT id, balance FROM users WHERE username = ?', [recipientUsername], (err, results) => {
        if (err || results.length === 0) {
        ...
        database.query('UPDATE users SET balance = balance - ? WHERE id = ?', [amount, req.user.id], (err) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }

            database.query('UPDATE users SET balance = balance + ? WHERE id = ?', [amount, recipientId], (err) => {
                if (err) {
                    return res.status(500).json({ error: err.message });
                }

                // Log transaction for both users
                database.query('INSERT INTO transactions (user_id, type, amount) VALUES (?, "transfer", ?)', [req.user.id, amount]);
                database.query('INSERT INTO transactions (user_id, type, amount) VALUES (?, "transfer", ?)', [recipientId, amount]);
